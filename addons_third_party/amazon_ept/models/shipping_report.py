...
class ShippingReportRequestHistory(models.Model):
    ...
    def default_get(self, fields):
        ...
        if fields:
            report_type = ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA if self.seller_id.is_european_region else \
                ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_US
    ...
    def search_or_create_partner(self, row, instance, partner_dict, country_dict, state_dict):
        ...
        if bill_country and row.get('bill-state', '') != '--':
            bill_state = res_partner_obj.create_or_update_state_ept(ship_country.code,
                                                                    row.get('bill-state', ''),
                                                                    ship_vals.get('zip', ''), ship_country)
        else:
            bill_state = ''
        ...
        partner_dict, invoice_partner = self.search_or_create_amz_invoice_partner(row, instance, bill_vals,
                                                                                  partner_dict)
    ...
    def request_shipment_report_by_software(self, seller_id, start_date, end_date):
        ...
        """
        report_type = ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA if \
            self.env['amazon.seller.ept'].browse(seller_id).is_european_region \
            else ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_US
    ...
    def search_or_create_amz_shipment_report(self, seller_id, report):
        ...
        report_type = ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA \
            if self.env['amazon.seller.ept'].browse(seller_id).is_european_region \
            else ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_US
        ...
        if seller_id:
            ...
            for report in ship_reports:
                ...
                if report.report_id and report.state in ['_DONE_', 'DONE'] and not report.attachment_id:
                    report.with_context(is_auto_process=True, amz_report_type='shipment_report_spapi').get_report()
    ...
    def get_reports_from_other_softwares(self, seller, start_date, end_date):
        ...
        report_type = ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA if seller.is_european_region else \
            ReportType.GET_AMAZON_FULFILLED_SHIPMENTS_DATA_US
